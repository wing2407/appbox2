    ...
    protected void onResume() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String url = "http://192.168.1.38/test.json";
                ...
                try {
                    ...
                    if (jsonArray != null) {
                        ...
                        for (int i = 0; i < jsonArray.size(); i++) {
                            ...
                            //循环获取apps_info
                            Request request_info = new Request.Builder().url("http://192.168.1.38/"+jsonArray.getString(i)+"/info").build();
                            ...
                            //根据每个ID的json接口获取对应的信息
                            Request request_icon = new Request.Builder().url("http://192.168.1.38/" + jsonArray.getString(i) + "/download.png").build();
                         ...
                         //if数据库中,该日期作为version的表存在则不创建，否则创建
                            DBHelper dbHelper = new DBHelper(getBaseContext(), "apps.db", null, day);
